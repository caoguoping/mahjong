
--------------------------------
-- @module SDKLoginData
-- @extend Ref
-- @parent_module 

--------------------------------
-- 
-- @function [parent=#SDKLoginData] readIp 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] readOpenid 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] readCity 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] readHeadimgurl 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] readNickName 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] readSex 
-- @param self
-- @return string#string ret (return value: string)
        
--------------------------------
-- 
-- @function [parent=#SDKLoginData] create 
-- @param self
-- @param #cc.EventCustom pEvent
-- @return SDKLoginData#SDKLoginData ret (return value: SDKLoginData)
        
return nil
